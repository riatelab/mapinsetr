...
[![mapinsetr status badge](https://riatelab.r-universe.dev/mapinsetr/badges/version)](https://riatelab.r-universe.dev/mapinsetr)
...
The `geodata` package offers an access to the [GADM database](http://www.gadm.org/) through the `gadm()` function. 
...
```{r dowload, fig.width=6, fig.height=6}
library(geodata)
library(sf)
library(mapsf)
...
This mask has to be an `sf` object. 
The `create_mask` function create a mask based on a `bbox`, an `sf` or `sfc` extent or an interactively defined rectangle.  
In this example we use a mask defined by Algiers wilaya extent + a 2 km buffer. 

```{r mask, fig.width=6, fig.height=6}
library(mapinsetr)
...
```{r createInset, fig.width=7, fig.height=5.5}
...
```{r mergespdf, fig.width=7, fig.height=6}
